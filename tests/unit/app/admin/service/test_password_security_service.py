...
async def test_handle_login_failure_increments_until_lock() -> None:
    db = AsyncMock()
    with patch("backend.app.admin.service.user_password_history_service.load_user_security_config", new_callable=AsyncMock):
        with patch("backend.app.admin.service.user_password_history_service.redis_client") as redis:
...
async def test_check_password_expiry_returns_days_when_near() -> None:
    ...
    with patch("backend.app.admin.service.user_password_history_service.load_user_security_config", new_callable=AsyncMock):
        with patch.object(settings, "USER_PASSWORD_EXPIRY_DAYS", 100), patch.object(
            settings, "USER_PASSWORD_REMINDER_DAYS", 30
        ):
