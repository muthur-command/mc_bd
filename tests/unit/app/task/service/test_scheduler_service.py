...
async def test_create_crontab_validates_expression() -> None:
    with patch("backend.app.task.service.scheduler_service.task_scheduler_dao") as dao:
        ...
        obj.crontab = "bad"
        with patch("backend.app.task.service.scheduler_service.crontab_verify", side_effect=errors.RequestError(msg="Crontab 表达式非法")):
...
async def test_execute_invalid_json_args() -> None:
    ...
    ts.args = "not-json"
    ts.kwargs = None
    with patch("backend.app.task.service.scheduler_service.run_in_threadpool", new_callable=AsyncMock, return_value=[{}]):
        with patch("backend.app.task.service.scheduler_service.task_scheduler_dao") as dao:
            dao.get = AsyncMock(return_value=ts)
            db = AsyncMock()
