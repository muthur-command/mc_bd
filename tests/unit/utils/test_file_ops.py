...
def test_upload_file_verify_image_too_large() -> None:
    ...
    f.filename = "x.image"
    f.size = 999
    with patch.object(settings, "UPLOAD_IMAGE_EXT_INCLUDE", ["image"]), patch.object(settings, "UPLOAD_IMAGE_SIZE_MAX", 10):
        with pytest.raises(errors.RequestError) as ei:
            upload_file_verify(f)
...
async def test_upload_file_writes_bytes(tmp_path) -> None:
    ...
    with patch("backend.utils.file_ops.UPLOAD_DIR", tmp_path), patch("backend.utils.file_ops.timezone.now", return_value=mock_now):
