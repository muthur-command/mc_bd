...
def main() -> None:
    ...
    if (
        arguments.commit
        and subprocess.run(["git", "diff", "--quiet"], check=False, cwd=_REPO_ROOT).returncode
        == 1
    ):
...
def test_bump_version(monkeypatch) -> None:
    ...
    monkeypatch.setattr(_mod, "_utcnow_compact", lambda: "201904241254")
