"""Docker 插件 API 单测：挂载 docker v1 + JWT + DB 依赖覆盖。"""

from __future__ import annotations

from typing import TYPE_CHECKING
from unittest.mock import AsyncMock, patch

import pytest

from fastapi import FastAPI
from starlette.middleware.authentication import AuthenticationMiddleware
from starlette_context.middleware import ContextMiddleware
from starlette_context.plugins import RequestIdPlugin

from backend.database.db import get_db, get_db_transaction
from backend.middleware.jwt_auth_middleware import JwtAuthMiddleware
from tests.helpers.admin_test_user import default_admin_jwt_user
from tests.helpers.fastapi_testing import ensure_rate_limiter_patched_for_tests, starlette_test_client

ensure_rate_limiter_patched_for_tests()

from backend.plugin.docker.api.router import v1 as docker_v1  # noqa: E402

if TYPE_CHECKING:
    from collections.abc import Generator

    from starlette.testclient import TestClient


@pytest.fixture
def docker_api_app() -> FastAPI:
    app = FastAPI()
    app.add_middleware(
        ContextMiddleware,
        plugins=[RequestIdPlugin(validate=False)],
    )
    app.add_middleware(
        AuthenticationMiddleware,
        backend=JwtAuthMiddleware(),
        on_error=JwtAuthMiddleware.auth_exception_handler,
    )
    app.include_router(docker_v1)

    async def _mock_db() -> AsyncMock:
        yield AsyncMock()

    async def _mock_db_tx() -> AsyncMock:
        yield AsyncMock()

    app.dependency_overrides[get_db] = _mock_db
    app.dependency_overrides[get_db_transaction] = _mock_db_tx
    return app


@pytest.fixture
def docker_api_client(docker_api_app: FastAPI) -> Generator[TestClient, None, None]:
    with starlette_test_client(docker_api_app, raise_server_exceptions=False) as c:
        yield c


@pytest.fixture
def docker_auth_headers(bearer_headers_admin_api: dict[str, str]) -> dict[str, str]:
    return bearer_headers_admin_api


@pytest.fixture(autouse=True)
def _docker_stub_jwt() -> Generator[None, None, None]:
    user = default_admin_jwt_user()
    with patch(
        'backend.middleware.jwt_auth_middleware.jwt_authentication',
        new_callable=AsyncMock,
        return_value=user,
    ):
        yield
