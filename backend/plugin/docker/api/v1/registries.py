...
async def list_registries(
    ...
    try:
        result = await db.execute(
            select(DockerConfig).where(DockerConfig.key == REGISTRY_CONFIG_KEY)
        )
        ...
        return response_base.success(
            data=[RegistrySourceResponse(**r) for r in registries]
        )
    except Exception as e:
        return response_base.fail(res=CustomResponse(code=400, msg=f'获取镜像源列表失败: {str(e)}'))
...
async def create_registry(
    ...
    try:
        ...
        await db.rollback()
        return response_base.fail(res=CustomResponse(code=400, msg=f'添加镜像源失败: {str(e)}'))
...
async def update_registry(
    ...
    try:
        ...
        await db.rollback()
        return response_base.fail(res=CustomResponse(code=400, msg=f'更新镜像源失败: {str(e)}'))
...
async def delete_registry(
    ...
    try:
        ...
        result = await db.execute(
            select(DockerConfig)
            .where(DockerConfig.key == REGISTRY_CONFIG_KEY)
            .with_for_update()
        )
        config = result.scalar_one_or_none()
        ...
        await db.rollback()
        return response_base.fail(res=CustomResponse(code=400, msg=f'删除镜像源失败: {str(e)}'))
