    ...
    try:
        ...
        if 'The command executed was' in error_msg:
            ...
            for line in lines:
                ...
                if (
                    line
                    and 'The command executed was' not in line
                    and 'returned with code' not in line
                    and 'stdout' not in line.lower()
                    and 'stderr' not in line.lower()
                    and 'stacktrace' not in line.lower()
                ):
                    useful_lines.append(line)

            error_msg = useful_lines[0] if useful_lines else f'Docker 命令执行失败 (镜像: {obj.image}:{obj.tag})'
...
async def build_image(obj: BuildImageParam) -> ResponseModel:
    ...
    try:
        ...
        return response_base.fail(res=CustomResponse(code=500, msg=f'构建镜像时发生未知错误: {e!s}'))
...
async def build_image_from_upload(
    ...
    try:
        ...
        # 验证文件类型（简单检查文件头）
        if (
            not tar_data.startswith(b'PK')
            and not tar_data.startswith(b'\x1f\x8b')
            and not tar_data.startswith(b'ustar')
        ):
