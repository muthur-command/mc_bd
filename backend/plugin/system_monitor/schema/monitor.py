"""监控相关 Schema（与 backend.app.admin.schema.monitor 一致，供插件自用）"""
...
class RedisServerInfo(SchemaBase):
    ...
    uptime_seconds: int = Field(description='运行时长（秒），供前端 i18n 格式化')
    uptime_in_seconds: str = Field(description='运行时长（已格式化，兼容旧版）')
