"""OAuth2 插件 API 单测：挂载 oauth2 v1 + JWT + DB / 事务依赖覆盖。"""
...
@pytest.fixture
def oauth2_api_client(oauth2_api_app: FastAPI) -> Generator[TestClient, None, None]:
    with starlette_test_client(oauth2_api_app, raise_server_exceptions=False) as c:
        yield c
...
@pytest.fixture(autouse=True)
def _oauth2_stub_jwt() -> Generator[None, None, None]:
